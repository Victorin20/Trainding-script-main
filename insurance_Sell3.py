...
blackbuyStop_stopLoss = 50
blackbuyStop_takeProfit = 75
blackSellLimit_takeProfit = 75
blackSellLimit_stopLoss = 50
one_lot = 10


path = "C:/Users/Public/insurance_Sell3.txt"
...
def sellLimit(position, volume):
    ...
    result = mt5.order_send(request)
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        print("Sell Limit Order placement failed. Error code:", result.retcode)
    else:
...
def blackbuyStop(position, volume):
    symbol = "EURUSD"  # EUR/USD currency pair
...
while failed == False and succes == False:
    ...
    if(failed == False):
        ...
        if(last_price <= startPrice - points*3 and fifth == False and fourd_active):
            blackbuyStop(2, (volume*16 * 25* one_lot / (blackbuyStop_stopLoss * one_lot)))
            fifth = True
